...
// Stress Test for superbank-rpc getBlockHeight
//
// Usage:
...
import { getBlockHeight } from '../../lib/rpc.js';
...
export const options = {
  ...
  thresholds: {
    ...
    rpc_getBlockHeight_latency: [`p(95)<${config.thresholds.stressP95Latency}`],
    rpc_error_rate: ['rate<0.15'],
  },
};

function buildBlockHeightOptions() {
  const options = {};
  if (config.blockHeightCommitment) {
    options.commitment = config.blockHeightCommitment;
  }
  if (config.blockHeightMinContextSlot !== null) {
    options.minContextSlot = config.blockHeightMinContextSlot;
  }
  return options;
}

export default function () {
  getBlockHeight(buildBlockHeightOptions());
}

export function handleSummary(data) {
  const summary = {
    testType: 'stress-get-block-height',
    timestamp: new Date().toISOString(),
    config: {
      rpcUrl: config.rpcUrl,
      commitment: config.blockHeightCommitment,
      minContextSlot: config.blockHeightMinContextSlot,
    },
    metrics: {
      requests: {
      ...
      latency: {
        avg: data.metrics.rpc_getBlockHeight_latency?.values?.avg || 0,
        p95: data.metrics.rpc_getBlockHeight_latency?.values['p(95)'] || 0,
        p99: data.metrics.rpc_getBlockHeight_latency?.values['p(99)'] || 0,
        max: data.metrics.rpc_getBlockHeight_latency?.values?.max || 0,
      },
      errors: {
